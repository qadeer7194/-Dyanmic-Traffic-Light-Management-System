...
class YOLO(object):
    _defaults = {
       ...
       # "model_path": 'model_data/yolo_weights1000.h5',           # 2-1) to test the raccoon_dataset_derived_model
        "anchors_path": 'model_data/yolo_anchors.txt',
        "classes_path": 'model_data/coco_classes.txt',         # 2-2) to test the raccoon_dataset_derived_model
...
#cv2.line(image, (image.shape[1]//3 *3, ROI_YMAX - 20), (image.shape[1]//3 *3, ROI_YMAX + 20), LIGHT_CYAN, 4, cv2.LINE_AA)
        ...
        cv2.putText(image, "Car:", ((((oW//3)//2)*3)-12, ROI_YMAX + 50), fontface, 1, LIGHT_CYAN, 2, cv2.LINE_AA)
        cv2.putText(image, "TWO WHEEL:", ((((oW//3)//2)*5)-80, ROI_YMAX + 50), fontface, 1, LIGHT_CYAN, 2, cv2.LINE_AA)

        cv2.putText(image, str(self.counter2), (((oW//3)//2)-5, ROI_YMAX + 100), fontface, 1, LIGHT_CYAN, 2, cv2.LINE_AA)
        cv2.putText(image, str(self.counter3), ((((oW//3)//2)*3)-5, ROI_YMAX + 100), fontface, 1, LIGHT_CYAN, 2, cv2.LINE_AA)
        cv2.putText(image, str(self.counter1), ((((oW//3)//2)*5)-5, ROI_YMAX + 100), fontface, 1, LIGHT_CYAN, 2, cv2.LINE_AA)
        #count1=self.counter2+self.counter3+self.counter1
...
def detect_video(yolo,yolo1,yolo2,yolo3, video_path, output_path="final_demo.mp4"):
   ...
   # vid3  = cv2.VideoCapture('phase3.mp4')
...
#3rd
    ...
    while True:
        ...
        #             font1Scale=0.50, color=(255, 0, 0), thickness=2)
        cv2.namedWindow("result", cv2.WINDOW_NORMAL)
        cv2.imshow("result", result)

        cv2.namedWindow("2nd", cv2.WINDOW_NORMAL)
        cv2.imshow("2nd",result1)

        cv2.namedWindow("3rd", cv2.WINDOW_NORMAL)
        cv2.imshow("3rd",result2)
